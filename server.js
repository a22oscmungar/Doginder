...
app.post('/users', upload.single('imagenFile'), (req, res) => {
    ...
    const { name, latitude, longitude, surname, mailUsu , pass} = req.body;
    ...
    if (!name || !latitude || !longitude || !surname || !mailUsu || !pass) {
    ...
    const query = `
    INSERT INTO USUARIO (nombreUsu, ubiUsu, apellidosUsu, mailUsu, imageUrl, pass) VALUES (?, ST_GeomFromText(?), (?), (?), ?, ?)
`;
    const values = [name, location, surname, mailUsu, imageUrl, pass];
