...
const db = mysql.createPool({
      ...
      if (!mailUsu || !passUsu) {
        ...
        db.query(sql, mailUsu, (err, result) => {
          if (err) {
            ...
            if (result.length === 0 || result[0].pass !== CryptoJS.MD5(passUsu).toString()) {
        ...
        if (err) {
          ...
          transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
              ...
              resolve({ message: "Correo electrónico enviado exitosamente", token: token.toString() });
            }
          });
        }
      });
  ...
  function changePassword(mail, pass, db) {
    return new Promise((resolve, reject) => {
      ...
      db.query(sql, values, (err, result) => {
        if (err) {
          console.error("Error en la consulta de actualización de contraseña:", err);
  ...
  function findNearbyUsers(currentLatitude, currentLongitude, radiusInKm, idUsu, db) {
    return new Promise((resolve, reject) => {
      const query = `
        SELECT 
            U.idUsu, 
  ...
  function findAllNearbyUsers(currentLatitude, currentLongitude, radiusInKm, idUsu, db) {
    return new Promise((resolve, reject) => {
      const query = `
        SELECT 
            U.idUsu, 
  ...
  function createUser(req) {
    return new Promise((resolve, reject) => {
      ...
      const imgProfile = req.files['imgProfile'][0]; // Imagen del usuario
      const imgPerfilFile = req.files['imgPerfilFile'][0]; // Imagen del perfil
      ...
      db.query(checkMatch, [idUsu2, idUsu1], (err, result) => {
        if (err) {
          ...
          if (result.length > 0) {
            ...
            db.query(noHabiaMatch, [idUsu1, idUsu2, tipoInteraccion], (err, result) => {
              if (err) {
                console.error("Error en la consulta de inserción:", err);
                reject({ error: "Error al agregar el usuario" });
              } else {
